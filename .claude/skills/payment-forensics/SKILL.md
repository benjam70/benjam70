...
- `mcp__Coralogix__query_dataprime` (plus `search_for_fields`, `search_labels`, `list_datasets`, `get_datetime`, `list_dataprime_commands`, `describe_dataprime_construct`, `show_dataprime_command_details`) — production logs and traces, queried directly against Coralogix. **Never use `mcp__Neo__coralogix_search_logs` instead, even as a fallback — it is off-limits in this skill, not just de-prioritized.** This isn't only a routing preference: a real case (order GE12346777794GB, a refund from roughly six weeks prior) came back completely empty from `mcp__Neo__coralogix_search_logs` across five different identifiers and a broad `applicationname:production` sanity check, then returned rich, detailed results immediately from `mcp__Coralogix__query_dataprime` on the identical query, time window, and order. The two tools have materially different retention or data-access scope; the Neo-routed one is not a reliable stand-in for the direct one and can produce a false "nothing found." If `mcp__Coralogix__query_dataprime` doesn't resolve in a given session, that is a Data Gap ("Coralogix not accessible: direct Coralogix MCP tools not present in this session") — do not fall back to the Neo-routed version to fill it. Use `search_for_fields` first when you don't already know the right field path — it beats guessing. **Every call requires an explicit `start_date`/`end_date` (ISO 8601 UTC), named exactly that, not `start_time`/`end_time`.** There is no implicit "search everything" default: omit both and the query silently narrows to a 15-minute window, which will look like a clean negative when it's actually just too short a range. Call `mcp__Coralogix__get_datetime` for the current UTC time, then pick a range wide enough for the question (the confirmed-working investigations documented below needed windows from a few days to ~2 weeks) rather than defaulting to something narrow. This is usually your primary source for gateway/order events when the analyst hasn't already pasted the raw export: order lifecycle events, API errors, webhook activity, capture/refund calls. Prefer this over asking the analyst to go paste logs manually. The query itself must start with a source clause, e.g. `source logs | filter $d ~ 'search term'` for full-text search across the whole log document. **Query syntax gotcha, confirmed by testing:** `$d.userData` and similar dotted field paths into the log body are not valid keypaths and fail *silently* (zero results, no error) rather than erroring, so a real gap looks identical to a broken query. Use `$d ~ 'search term'` for full-text search across the whole log document instead. Label fields work as dotted paths (`$l.applicationname`, `$l.subsystemname`) since those are structured; the JSON payload itself is not. Adyen's webhook notifications land under `Source: PaymentsController`, `action: PSPNotificationHandler`, with an `EventCode` field (AUTHORISATION, CAPTURE, REFUND, etc.) inside that text-searchable body — confirmed present in both `qa` and `production` labeled logs. `list_dataprime_commands` / `describe_dataprime_construct` / `show_dataprime_command_details` cover DataPrime command/function syntax directly when the shape of a query isn't obvious.
  **Local fallback: `mcp__coralogix__coralogix_search_fields` (lowercase server name, distinct from the hosted `mcp__Coralogix__*` connector's `search_for_fields`).** When only the local Coralogix MCP server resolves in a session (`mcp__coralogix__coralogix_query_dataprime`, no hosted connector), it also ships a sibling field-discovery tool wrapping `cx search-fields`. Two modes: `semantic` (default) takes a plain-English description ("refund amount", "authorization failure status") and returns ranked candidate `$d.<path>` fields with a description of what each holds, confirmed working live, this is what to reach for instead of guessing a dotted path from memory. `value` mode instead takes a literal value you already expect ("AutherizationFailed") and finds which field carries it, but this needs a Team-scoped Coralogix API key; a Personal key or OAuth session fails it with a "missing team ID" auth error, that is a credential-scope gap to raise with whoever administers the Coralogix account, not evidence the field doesn't exist. Prefer describing the concept over the literal value when only semantic mode is available.
