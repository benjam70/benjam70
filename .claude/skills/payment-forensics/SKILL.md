...
- `mcp__Neo__coralogix_search_logs` — production logs and traces. This is usually your primary source for gateway/order events when the analyst hasn't already pasted the raw export: order lifecycle events, API errors, webhook activity, capture/refund calls. Prefer this over asking the analyst to go paste logs manually. **Query syntax gotcha, confirmed by testing:** `$d.userData` and similar dotted field paths into the log body are not valid keypaths and fail *silently* (zero results, no error) rather than erroring, so a real gap looks identical to a broken query. Use `$d ~ 'search term'` for full-text search across the whole log document instead. Label fields work as dotted paths (`$l.applicationname`, `$l.subsystemname`) since those are structured; the JSON payload itself is not. Adyen's webhook notifications land under `Source: PaymentsController`, `action: PSPNotificationHandler`, with an `EventCode` field (AUTHORISATION, CAPTURE, REFUND, etc.) inside that text-searchable body — confirmed present in both `qa` and `production` labeled logs. **Large-result handling, mandatory, don't improvise it inline:** a broad or multi-day query routinely blows the tool's own token limit and gets dumped to a local `.txt` file instead of returned inline, one raw Coralogix entry per line, often 10-20k+ characters of escaped JSON per line. Neither Read (offset/limit chunks by line, and one line alone can exceed the token cap) nor Grep (silently replaces a long matching line with `[Omitted long matching line]`) can pull a match out of a file like that. Don't write a fresh one-off Python regex scan for this each time it happens, it already exists: run `python3 <skill_dir>/scripts/coralogix_extract.py <saved_file> '<search term>' --context 200` (skill_dir is this SKILL.md's own directory). It prints each match with surrounding context and, when present on the same line, the Coralogix `timestamp` metadata value, ready to drop into a Timeline row. Narrow the initial query first regardless (tight time window, `| limit N`) since a smaller result is always cheaper than post-processing a large one, but when a result still lands as a file, use the script rather than ad hoc extraction. **Coverage gap on older dates, confirmed empirically:** this Neo wrapper sometimes returns zero results for a date range that genuinely has data in Coralogix (older dates in particular), where the underlying `mcp__Coralogix__query_dataprime` tool (direct Coralogix MCP server, not the Neo wrapper) reaches the same data successfully. If a `coralogix_search_logs` query for a plausible order/date comes back empty, don't conclude the data doesn't exist — retry the same DataPrime query (`$d ~ 'search term'` syntax carries over) via `mcp__Coralogix__query_dataprime` before writing it up as a Data Gap. Use whichever tool actually returns data going forward once you've confirmed one reaches something the other can't for that time range.
