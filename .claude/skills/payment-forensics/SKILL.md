...
**Merchant-side delivery is a separate question from issuance.** When `CreateOrderRefund` confirms a `RefundId` was issued, that only proves Global-e's own system processed the refund — it doesn't prove the merchant's system received it. Global-e also sends a `NotifyOrderRefund` webhook to the merchant's own endpoint for each refund. If a merchant claims "the customer says no refund arrived," check whether this webhook was actually delivered before assuming the refund itself failed — a confirmed `RefundId` with a failed or missing webhook delivery is a merchant-notification gap, not a refund-issuance gap, and the two should not be conflated in a finding.
