...
- **Search on the shipping address or full name, not a bare surname or the decimal amount.** A surname alone (e.g. `Strang`) is a common enough substring to false-match unrelated orders, six false hits in one real case, and an item SKU alone is no better if the item is popular: the same SKU search surfaced a completely different customer's order for the same product. A street address token (e.g. `Butterick`) or a full name string (e.g. `McVicar`) is not redacted and reliably surfaces the right `orderId` field nearby. Card last4 and merchant name are decent secondary tokens for the same reason.
- **Don't rule out email either, just aim it at the right log lines.** Order-*creation* payloads mask the customer email as `[***]` for PII, so a redacted-payload email search does return false-positive-looking noise, that part still holds. But transactional/notification events (order confirmation, RMA created, refund notification) log the real recipient address in plaintext to actually send the mail. When a surname or SKU search comes back noisy or ambiguous, searching the literal email address next is often the cleanest single token available, confirmed in a real case: it returned one consistent order ID across confirmation, RMA, and refund events, instead of the pile of unrelated look-alikes the surname and SKU searches produced. Don't trust a single loose token in isolation, cross-check whatever candidates surface against each other before committing to one.
