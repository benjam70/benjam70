...
- `mcp__Coralogix__query_dataprime` (plus `search_for_fields`, `search_labels`, `list_datasets`, `get_datetime`, `list_dataprime_commands`, `describe_dataprime_construct`, `show_dataprime_command_details`) — production logs and traces, queried directly against Coralogix. **Never use `mcp__Neo__coralogix_search_logs` instead, even as a fallback — it is off-limits in this skill, not just de-prioritized.** This isn't only a routing preference: a real case (order GE12346777794GB, a refund from roughly six weeks prior) came back completely empty from `mcp__Neo__coralogix_search_logs` across five different identifiers and a broad `applicationname:production` sanity check, then returned rich, detailed results immediately from `mcp__Coralogix__query_dataprime` on the identical query, time window, and order. The two tools have materially different retention or data-access scope; the Neo-routed one is not a reliable stand-in for the direct one and can produce a false "nothing found." If `mcp__Coralogix__query_dataprime` doesn't resolve in a given session, that is a Data Gap ("Coralogix not accessible: direct Coralogix MCP tools not present in this session") — do not fall back to the Neo-routed version to fill it. Use `search_for_fields` first when you don't already know the right field path — it beats guessing. **Every call requires an explicit `start_date`/`end_date` (ISO 8601 UTC), named exactly that, not `start_time`/`end_time`.** There is no implicit "search everything" default: omit both and the query silently narrows to a 15-minute window, which will look like a clean negative when it's actually just too short a range. Call `mcp__Coralogix__get_datetime` for the current UTC time, then pick a range wide enough for the question (the confirmed-working investigations documented below needed windows from a few days to ~2 weeks) rather than defaulting to something narrow. This is usually your primary source for gateway/order events when the analyst hasn't already pasted the raw export: order lifecycle events, API errors, webhook activity, capture/refund calls. Prefer this over asking the analyst to go paste logs manually. The query itself must start with a source clause, e.g. `source logs | filter $d ~ 'search term'` for full-text search across the whole log document. **Query syntax gotcha, confirmed by testing:** `$d.userData` and similar dotted field paths into the log body are not valid keypaths and fail *silently* (zero results, no error) rather than erroring, so a real gap looks identical to a broken query. Use `$d ~ 'search term'` for full-text search across the whole log document instead. Label fields work as dotted paths (`$l.applicationname`, `$l.subsystemname`) since those are structured; the JSON payload itself is not. Adyen's webhook notifications land under `Source: PaymentsController`, `action: PSPNotificationHandler`, with an `EventCode` field (AUTHORISATION, CAPTURE, REFUND, etc.) inside that text-searchable body — confirmed present in both `qa` and `production` labeled logs. `list_dataprime_commands` / `describe_dataprime_construct` / `show_dataprime_command_details` cover DataPrime command/function syntax directly when the shape of a query isn't obvious. **Second query syntax gotcha, confirmed by testing:** a compound filter mixing `||`/`&&` with a negated regex match (`!~`) returned a hard `400 Bad Request` from the DataPrime API, not a silent empty result. That's actually the easier failure to catch, a hard error says the query itself is broken rather than looking like a clean negative, but the fix is the same either way: drop the compound expression and run two plain single-condition filters instead of debugging the exact invalid token.
  **No working fuzzy (typo-tolerant) search exists here, confirmed by direct testing, don't spend time chasing one.** Coralogix's docs describe `$d ~~ 'term'` as a "fuzzy" all-fields search, but it's substring matching, not edit-distance tolerant: searching a correctly-spelled real value returns real hits, searching a deliberately misspelled string with no real match anywhere in the data returns zero, exactly what plain substring matching does, not what fuzzy matching would do. The `lucene` command (real Lucene syntax embedded in a DataPrime pipeline, e.g. `source logs | lucene 'finalTransactionStatus:AuthorizationFailed'`) works fine for plain field:value matching, confirmed live, but Lucene's own native fuzzy suffix (`term~`, `term~1`) returns a hard `400 Bad Request` the moment it's added, with the same field and value, in the same time window, that succeeds without it. That rules out a data gap or a window problem. The fuzzy suffix itself is what's unsupported here, on this account or this API tier. Don't try to work around a suspected spelling variant with a fuzzy operator, use the `system/engine.schema_fields` catalog above instead, it shows the actual spelling that exists rather than guessing at how far off a typo might be.
  ...
  **Local-only: `mcp__coralogix__coralogix_fuzzy_correct`, for a term you're not fully sure is spelled correctly.** The "no working fuzzy search" limit above still stands, there is no server-side fix for a typo. This tool closes the gap client-side using the `rapidfuzz` library instead: give it a term plus a list of real candidate values you already pulled from Coralogix (the `examples` column of a `system/engine.schema_fields` row, or a `coralogix_search_fields` value-mode result), and it ranks those candidates by true edit distance. Confirmed live: `AuthorizationFailed` against a candidate list that includes the real, oddly-spelled `AutherizationFailed` ranks that value first at 95% similarity. It only re-ranks candidates you already retrieved, it never invents a value that isn't in the list you gave it.
  **Local-only: `mcp__coralogix__coralogix_mine_templates`, to check whether a keyword hit is a real recurring event or a one-off false positive.** This skill already documents real false positives from keyword search alone: Revolut matched unrelated Stripe records, and Amazon Pay matched an internal code-review bot's log that happened to reuse the same class and method names as a real webhook handler (tabby's capitalization gap, elsewhere in this file, is the same underlying problem). This tool automates the check that eventually caught each of those: it clusters a batch of raw log message lines (the `message`/`Message` field from a broad `query_dataprime` pull, not a full JSON row) by structural shape, using the Drain algorithm. A genuine PSP event recurs across orders in a recognizable shape with only the order ID/amount/status varying; a coincidental keyword hit is usually alone, forming a cluster of one, flagged `likely_noise: true`. Confirmed live against a recreation of the Amazon Pay case: three genuine "Operation Type" confirmation lines merged into one template with only the operation type masked as the variable, and a fabricated bot-log line landed alone in its own cluster and was flagged as noise. Also useful for a not-yet-investigated gateway: pull a broad sample of raw lines for its route and mine templates directly to see the actual recurring message shapes at once, instead of testing field-name guesses one at a time.
  **Local-only: `mcp__coralogix__coralogix_infer_schema`, for a gateway whose full raw JSON webhook body is logged (Stripe, Checkout.com, PayJustNow are confirmed examples).** Every one of those gateways got documented the same way, by reading a handful of pasted example payloads by hand to work out which fields exist and which only show up on certain event types, e.g. PayJustNow's `refundAmount`/`refundReason` only appearing on a refund, never on an auth confirmation. This tool automates that read: give it a batch of raw JSON payloads already pulled from Coralogix and it merges them, separating fields present in every sample from fields that only show up on some. Confirmed live on a simulated PayJustNow-shaped batch: `checkoutPaymentStatus`, `paymentReference`, and `merchantOrderReference` correctly flagged `always_present: true`, `refundAmount`/`refundReason` correctly flagged `always_present: false` with a real example value attached. Complements `coralogix_mine_templates` rather than overlapping it: that tool clusters the free-text log message, this one merges the structured JSON body, use whichever matches the evidence shape a gateway actually logs.
  **When `coralogix_query_dataprime` saves a result to a file instead of returning it inline (large result), query that file with `python tools/query_coralogix_dump.py <path> --group-by <field> [<field> ...]` instead of grepping escaped JSON by hand.** Confirmed live on a real 10,344-line saved dump: one command returned the complete, untruncated set of distinct `message` values and their exact counts, the same information that previously took several rounds of regex tuning to extract and still risked truncating a message at an embedded escaped quote. Add `--sql "<query>"` instead of `--group-by` for anything needing a real WHERE clause or join; the saved dump is exposed as an `events` view with one `user_data` column holding each row's raw JSON, extract a field with `json_extract_string(user_data, '$.<field>')`. Requires the optional `duckdb` package (`tools-requirements.txt`); without it, fall back to grep on the saved file the way every case before this was actually investigated.
