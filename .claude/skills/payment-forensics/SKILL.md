...
**Worldpay disputes are a separate, unsolved problem.** They arrive via a scheduled **SFTP file drop**, not any API or webhook — there's a dedicated internal HLD for this ("WorldPay - collect chargebacks via SFTP files"). That means Coralogix is not confirmed to have Worldpay dispute data the way it does for Worldpay refunds or Adyen webhooks — a chargeback that only exists in a batch file, ingested on a schedule into (presumably) a DB table, may not appear as a request Coralogix ever logged. This is an open gap, not a solved one: if a real case needs a Worldpay dispute, don't assume Coralogix has it — say so as a Data Gap and ask the analyst where the SFTP-ingested table lands, or have them paste the record directly.
