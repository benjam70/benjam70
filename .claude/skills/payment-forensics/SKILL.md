...
- **`DATALAKE.CHARGEBACKS.CB_FACT_CHARGEBACKS`, found 2026-08-10 — query this first for any dispute/chargeback question, before reaching for a gateway-specific workaround.** It's a unified view across gateways (`PAYMENT_GATEWAY_NAME`, `STATUS`, `STATUS_MAPPED`, `REASON_CODE`, `REASON_DESCRIPTION`, `CATEGORY`, `CB_AMOUNT`/`CB_CURRENCY`, `ORDER_ID`, `MERCHANT_ORDER_ID`, `CHARGEBACK_SOURCE_SYSTEM`, `IS_LAST_EVENT`), sitting on top of per-gateway feeder tables in the same `CHARGEBACKS` schema (`CB_PAYPAL`, `CB_PAYPAL_JP`, `CB_STRIPE`, `CB_WORLDPAY`, `CB_ADYEN_20250819`, `CB_DLOCAL`, `CB_DLOCAL_S3`) plus `CHARGEBACKS_MGMT_MONTHLY_RAW` / `CHARGEBACKS_MGMT_REVERSALS_RAW`, which match the field layout of the manually-run "Monthly Chargebacks Report" — meaning even that manual report's output already lands in a queryable table, not just an analyst's inbox. This is separate from `DATALAKE.POSTGRESQL.DISPUTES` (the live-webhook pipeline table used for Adyen/Stripe/Klarna, see the Klarna disputes note below) — `CHARGEBACK_SOURCE_SYSTEM` on `CB_FACT_CHARGEBACKS` should indicate which underlying pipeline a given row came from, so check both this view and `POSTGRESQL.DISPUTES` rather than assuming one supersedes the other. **Not yet queried against real data** — confirmed to exist via schema search only, blocked on the same missing Snowflake PAT as everything else in this section. No `CB_KLARNA` table was found, consistent with Klarna's dispute data arriving live via webhook rather than a periodic file.
...
- `paypal_get_order` / `paypal_get_capture` / `paypal_get_refund` / `paypal_get_authorization` / `paypal_get_dispute` / `paypal_search_disputes` — direct PayPal lookups. Covers the V2 REST integration only, not legacy V1/NVP transactions. PayPal disputes route through their own path at Global-e (not the Justt chargeback pipeline other gateways use). **Correction, 2026-08-10: these MCP tools are not the only way to reach PayPal dispute detail** — see the CB_FACT_CHARGEBACKS entry below, and PayPal V2's own internal webhook storage path (Confluence PD "CORE-133827 Save PayPal V2 Disputes Notifications": `PayPalNotificationHandler` stores `CUSTOMER.DISPUTE.CREATED/UPDATED/RESOLVED` into a PaymentNotifications table, shipped with its own PagerDuty/Grafana/Coralogix monitoring, confirmed live in production traffic on `paymentGatewayId 25` as of 2026-07-25 — though no actual dispute event was caught in a 3-month Coralogix search, so treat "live" as confirmed, "has recent dispute volume" as not).

These six (Zendesk, Datadog, HubSpot, Monday, Stripe, PayPal) are project-local MCP servers, not built-in Neo tools, and each one only works once its own credential env var is actually set for the session (see `mcp-servers/README.md`). If a tool call fails with an auth error, that means the credential isn't provisioned yet, not that the system has no data — treat it exactly like the Snowflake PAT case below: a Data Gap with the specific reason stated, never silently skipped and never worked around.
...
**Klarna disputes: corrected 2026-08-10, this was wrong. There IS an automated path, and it is already live in production.** The earlier claim in this doc (no webhook, no API, monthly manual Klarna-portal export only) was based on an incomplete read of the "Payment API Mapping" doc — it missed the *generic* chargeback endpoint (`POST /Payments/PSPNotificationHandler/{paymentGatewayId}`), separate from the Justt-only endpoint (`POST /Payments/JusttNotificationHandler`). Justt genuinely does not cover Klarna (it's card-network-chargeback-only, confirmed), but Klarna does not need Justt: it has its own webhook straight into the generic pipeline, same as Adyen and Stripe. `PaymentsController.HandleChargebackNotification` dispatches the notification to `KlarnaDirectGateway.ParseGatewayChargebackNotification`, which composes a unified status (`GenerateUnifiedKlarnaDisputeStatus`) and publishes it for enrichment into the payments-management microservice's dispute tables. Confirmed live and firing: a Coralogix production search (`applicationname:production`, `HandleChargebackNotification` + `Klarna`, 2026-08-05 to 2026-08-10) returned real hits, e.g. a 2026-08-07 event with `paymentGatewayId:13`, `method: PaymentsController.HandleChargebackNotification`, `IsV4: false`. Confluence PD "CORE-218498: Handle Klarna Disputes API v4" (linked from Jira CORE-210659) documents the v3 pipeline as already production, plus an in-flight migration to Klarna's newer Disputes API v4, gated behind LaunchDarkly flag `core-210659-use-klarna-disputes-api-v4` (still evaluating `False` as of the log above, so production is still on v3 status mapping — don't assume v4 fields are populated yet).

The enriched result lands in queryable tables: **Production Warehouse (Snowflake) → `DATALAKE.POSTGRESQL.DISPUTES`** (21 columns, including `PAYMENT_GATEWAY_NAME`, `LATEST_DISPUTE_STATUS_RAW`, `LATEST_DISPUTE_STATUS_ID`, `DISPUTE_CATEGORY_ID`, `PAYMENT_PSP_REFERENCE`, `DISPUTE_PSP_REFERENCE`, `LATEST_LOGICAL_POSITION_IN_DISPUTE_FLOW`, `DATE_LAST_UPDATED`), plus lookup tables `DISPUTE_STATUSES`, `DISPUTE_STATUS_GROUPS`, and `DISPUTE_HISTORIES` for the audit trail. This is reachable via `neo_query_sql` against Production Warehouse, filtering `PAYMENT_GATEWAY_NAME` for Klarna (confirm the exact stored string, e.g. `KlarnaDirect`, before relying on it — not yet verified against a real row). **Not yet fully verified**: querying this table was blocked in this session by the same missing personal Snowflake access token already documented elsewhere in this doc, so no actual Klarna dispute row has been pulled and eyeballed yet — that's the next concrete step, not a re-open of the "no automated path" conclusion, which is now confirmed false. Once a PAT is available, spot-check a known Klarna dispute case end to end (webhook in Coralogix → row in `DATALAKE.POSTGRESQL.DISPUTES`) the same way Adyen was verified.

For a live Klarna dispute case in the meantime: try `neo_query_sql` against `DATALAKE.POSTGRESQL.DISPUTES` first (if a PAT is configured for the session), fall back to Coralogix (`HandleChargebackNotification`, filter by order/PSP reference) to confirm the webhook arrived, and only fall back to "ask the analyst to check Klarna's merchant portal" if neither surfaces the case — that manual monthly export is a reporting process for a different purpose (a periodic reconciliation report), not evidence that no queryable source exists.
...
**Worldpay disputes: revisited 2026-08-10, partially corrected — do not repeat the Klarna mistake of assuming "SFTP, therefore no queryable table."** They arrive via a scheduled SFTP file drop, not any API or webhook, so Coralogix genuinely won't have them (that part holds — a file drop makes no HTTP request for Coralogix to log). But checking the Production Warehouse schema (not just Coralogix) turned up four tables that are unmistakably the raw WorldPay chargeback file format: `DATALAKE.WORLDPAY.WP_CBK`, `WP_CBK_NEW`, `WP_GLOE_CBK_INC`, `WP_GLO2_CBK_INC` — each with `ORDER_CODE`, `CHARGEBACK_DATE`, `CHARGEBACK_REASON_CODE`, `DISPUTE_ID`, `DISPUTE_CASE_ID`, `ARN`, `CHARGEBACK_CYCLE`. Some raw ingestion of the SFTP files into Snowflake already exists, independent of the "single source of truth" pipeline.

Don't overclaim this the way the old text overclaimed the opposite. The internal HLD to wire Worldpay into the same unified chargeback pipeline Adyen/Stripe/Klarna use ("HLD - WorldPay - collect chargebacks via SFTP files," CORE-178299) is still a draft: three competing architecture options, decision log and open-points sections both empty, no option chosen. So a normalized, enriched Worldpay row in `DATALAKE.POSTGRESQL.DISPUTES` alongside the other gateways does **not** exist yet. What the `WP_CBK*` tables actually contain, whether they're still being actively fed, and which of the four is current versus a stale/superseded variant, is **not confirmed** — checking was blocked by the same missing Snowflake personal access token noted elsewhere in this doc. Treat those four table names as a lead to try first (`neo_query_sql` filtered by `ORDER_CODE` or `DISPUTE_ID`), not a confirmed source — if a query against them comes back empty or clearly stale, that's real evidence for a Data Gap; not having tried them isn't.

**What you still do not have here:** Outlook, Teams, and SharePoint have no MCP tool in this environment. If a case genuinely needs one of them, that is exactly what RULE 0's `RAW COPY CASE CAPTURE` path is for: treat it as something the analyst pastes in, not something to silently skip, and don't fabricate what that content might say.
