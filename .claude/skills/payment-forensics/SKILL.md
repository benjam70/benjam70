...
- `mcp__Coralogix__query_dataprime` — production logs and traces, queried directly against Coralogix (not proxied through Neo). This is usually your primary source for gateway/order events when the analyst hasn't already pasted the raw export: order lifecycle events, API errors, webhook activity, capture/refund calls. Prefer this over asking the analyst to go paste logs manually. **Every call requires an explicit `start_date`/`end_date` (ISO 8601 UTC).** Unlike the old Neo-mediated tool, there is no implicit "search everything" default: omit both and the query silently narrows to a 15-minute window, which will look like a clean negative when it's actually just too short a range. Call `mcp__Coralogix__get_datetime` for the current UTC time, then pick a range wide enough for the question (the confirmed-working investigations documented below needed windows from a few days to ~2 weeks) rather than defaulting to something narrow. The query itself must start with a source clause, e.g. `source logs | filter $d ~ 'search term'` for full-text search across the whole log document. **Query syntax gotcha, confirmed by testing (still applies, same underlying query language):** `$d.userData` and similar dotted field paths into the log body are not valid keypaths and fail *silently* (zero results, no error) rather than erroring, so a real gap looks identical to a broken query. Label fields work as dotted paths (`$l.applicationname`, `$l.subsystemname`) since those are structured; the JSON payload itself is not. Adyen's webhook notifications land under `Source: PaymentsController`, `action: PSPNotificationHandler`, with an `EventCode` field (AUTHORISATION, CAPTURE, REFUND, etc.) inside that text-searchable body — confirmed present in both `qa` and `production` labeled logs. When a field name or query shape isn't obvious, `mcp__Coralogix__search_for_fields` finds candidate field paths by description or example value instead of guessing, and `mcp__Coralogix__list_dataprime_commands` / `describe_dataprime_construct` / `show_dataprime_command_details` cover DataPrime command/function syntax directly, so there's no need to fall back to trial and error on query shape.
...
These six (Zendesk, Datadog, HubSpot, Monday, Stripe, PayPal) are project-local MCP servers, not built-in Neo tools, and each one only works once its own credential env var is actually set for the session (see `mcp-servers/README.md`). Coralogix is a separate case: it's its own directly-connected MCP server (`mcp__Coralogix__*`), not a project-local server from this repo and not proxied through Neo either, so the credential/env-var caveat below doesn't apply to it the same way — but the general point still does: before relying on it, confirm the tool actually resolves in this session rather than assuming it's callable. Before relying on any of the project-local six, confirm the tool actually resolves in this session (search for it, don't assume the name from this list is callable). In some environments none of these six exist at all, not merely lacking a credential, and that is a materially different fact worth stating precisely, not blurring together:
...
**Adyen and Klarna have no MCP tool, deliberately.** Both are inbound-webhook-driven at Global-e (`PSPNotificationHandler` / `KlarnaDirectController.HandleNotification`), not query-by-reference, so a live "look up this payment" tool isn't the right shape for how the data actually gets there. Klarna's only outbound call is `KlarnaDirectGateway.ReadHPPSession(...)`, narrowly scoped to finalizing a redirect, not general lookup — same category of narrow-and-not-useful-for-investigation as Adyen's `PaymentResultRequest`. Use `mcp__Coralogix__query_dataprime` for both instead.
