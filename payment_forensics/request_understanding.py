...
from .thread_digest import ThreadDigest, build_thread_digest
...
class TicketUnderstanding:
    ...
    thread_digest: Mapping[str, Any] = field(default_factory=dict)
...
def understand_ticket(text: str, thread_history: Iterable[str | Mapping[str, Any]] = ()) -> TicketUnderstanding:
    ...
    value = str(text or "")
    history_list = list(thread_history)
    history_values = [str(item.get("text", "")) if isinstance(item, Mapping) else str(item) for item in history_list]
    ...
    if any(term in lowered for term in ("chargeback", "dispute outcome", "still open")):
        intents.append("confirm_chargeback_outcome")
        artifacts.append("chargeback_outcome")
        requested.append("chargeback_outcome")
    ...
    if "chargeback_outcome" in unique_artifacts:
        questions.append("Was the chargeback outcome answered?")
    ...
    audience = "merchant" if "merchant" in lowered else "internal_cs"
    digest = build_thread_digest(value, history_list, recipient=audience)
    digest_known = tuple(sorted(digest.fact_keys_for_recipient()))
    known_facts = tuple(dict.fromkeys(known + digest_known + (previous.known_facts if previous else ())))
    confidence = 0.95 if len(unique_intents) > 1 or unique_artifacts else 0.75
    explicit = tuple(dict.fromkeys(requested))
    unanswered = tuple(dict.fromkeys(list(questions) + list(digest.open_items)))
    if digest.open_items or digest_known:
        acts = list(dict.fromkeys([*acts, "avoid_repetition"]))
...
def validate_ticket_answer(understanding: TicketUnderstanding, output: str) -> tuple[str, ...]:
    ...
    if "chargeback_outcome" in understanding.requested_artifacts and not any(
        term in lowered for term in ("chargeback", "dispute", "reversed", "pending", "pre-arbitration", "prearbitration")
    ):
        reasons.append("ticket requested chargeback outcome but the draft does not mention the dispute outcome")
