...
Colon-explainer construction: watch "X: here's why" or "X: this happened because Y," a claim followed by a colon-introduced explanation clause. Confirmed AI tell (Wikipedia:Signs of AI writing). Split into two sentences instead: state the claim, then state the reason, plainly.
Participial tack-on: watch a sentence that states a fact then appends a present-participle clause to smuggle in the conclusion ("X happened, ruling out Y" / "X happened, confirming Y"). Split into two sentences: state the fact, then state the conclusion as its own sentence.
Semicolon and em-dash overuse: AI writing leans on both far more than human writing does, and rarely uses an ellipsis. A semicolon joining two independent clauses is often just two sentences that got glued together for false cohesion, split them.
Formal connective overuse: "furthermore," "moreover," "consequently," "additionally" showing up in more than one sentence in a short passage is a tell on its own account, per the same source. Human writing mostly just starts the next sentence.
```
