# MCP servers: Dudley and evidence connectors

## Dudley: host-operated investigation service

`dudley/server.py` follows the host-client pattern: Codex or Claude Code is
the reasoning client, while Dudley exposes its canonical methodology, durable
case state, and deterministic completion gate through MCP. Dudley does not
call or bundle a local model, and the server does not modify the Mother Engine.

It provides the `dudley://methodology` resource, an
`investigate_payment_case` prompt, durable case tools, and
`dudley_check_completion_gate`, backed by the same controller gate as
`tools/dudley_gate_check.py`.

Run it with `python mcp-servers/dudley/server.py`. Codex and Claude Code can
spawn it from `.mcp.json`. The host calls the separate read-only evidence MCP
servers; Dudley never converts an unavailable source into a false empty result.
...
This repo does now register a *local* `coralogix` server in `.mcp.json`
(`mcp-servers/coralogix/server.py`, using `CORALOGIX_API_KEY` /
`CORALOGIX_DOMAIN`), as a fallback for when the hosted connector isn't
granted. See `docs/coralogix-claude-code.md` for the full connect runbook:
CLI vs MCP setup, credential/scope requirements, and the current known
failure modes (IP allowlisting, OAuth profiles missing query scope) to check
before assuming the account has no Coralogix access at all.
